...
function openModal() {
  document.getElementById("inciModal").style.display = "flex";
}

function closeModal() {
  document.getElementById("inciModal").style.display = "none";
}
...
function analizarIngredientes() {
  ...
  ingredientes.forEach(ingrediente => {
    const data = ingredientesDB[ingrediente];
    ...
    if (data) {
      card.innerHTML = `
        <h3>${ingrediente}</h3>
        <p class="verde tag">🟢 ${data.beneficio}</p>
        <p class="amarillo tag">🟡 ${data.advertencia}</p>
        <p class="rojo tag">🔴 ${data.riesgo}</p>
      `;
    } else {
      card.innerHTML = `
        <h3>${ingrediente}</h3>
        <p>⚠️ No existe suficiente evidencia científica disponible.</p>
      `;
    }
